...
fn setup_cameras(mut commands: Commands, mut viewports: GtkViewports) {
    ...
    let main_camera = commands
        .spawn((
            ...
            viewports.create(GstWebRtcSettings {
...
fn setup_cameras_2(mut commands: Commands, mut streamer: StreamerHelper<GstWebRtcEncoder>) {
    // camera
    let main_camera_2 = commands
        .spawn((
            Camera::default(),
            Camera3d::default(),
            Transform::from_xyz(-2.5, 4.5, 9.0).looking_at(Vec3::ZERO, Vec3::Y),
            streamer.new_render_target(GstWebRtcSettings {
                signalling_server: SignallingServer::PixelStreaming {
                    uri: "ws://localhost:8988".to_string(),
                    streamer_id: Some("player_copy".to_string()),
                },
                // signalling_server: SignallingServer::GstWebRtc {
                //     uri: "ws://127.0.0.1:8443".to_string(),
                //     peer_id: None,
    ...
    commands.spawn((
        Text::new("Camera with GPU => CPU => GPU."),
        Node {
            position_type: PositionType::Absolute,
            top: Val::Px(12.0),
            left: Val::Px(12.0),
...
fn update_player_position_and_spectator_view(
    ...
    let camera_transform = q_player_camera_transform.iter().next().unwrap();
