...
use gst::prelude::AllocatorExt;
use gst_allocators::DmaBufAllocatorExtManual;
use {
    arrayvec::ArrayVec,
    ash::vk,
    bevy_app::prelude::*,
...
impl DmabufTexture {
    ...
    pub fn build_caps(&self) -> Result<gst::Caps, BevyError> {
        let video_info_dma_drm = VideoInfoDmaDrm::new(
            gst_video::VideoInfo::builder(
                // dma_drm_fourcc_to_format(self.drm_format.code as u32)?,
                // VideoFormat::Rgba,
                VideoFormat::DmaDrm,
                self.width(),
                self.height(),
            ...
            .build()?,
            self.drm_format.code as u32,
            self.drm_format.modifier.into(),
        );

        // let mut caps = gst_video::VideoInfo::builder(
        //     // dma_drm_fourcc_to_format(self.drm_format.code as u32)?,
        //     // VideoFormat::Rgba,
        //     VideoFormat::DmaDrm,
        //     self.width(),
        ...
        //
        // {
        //     let format = self.drm_format.code as u32;
        //     let modifier: u64 = self.drm_format.modifier.into();
        //     // let modifier = modifier as i64;
        //     let caps_ref = caps.make_mut();
        //     caps_ref.set("drm-format", "AB24");
        //     caps_ref.set("drm-modifier", modifier);
        //     caps_ref.set_features_simple(Some(CapsFeatures::new(["memory:DMABuf"])));
        // }
        //
        // Ok(caps)

        Ok(video_info_dma_drm.to_caps()?)
    }

    pub fn build_gst_buffer(&self) -> Result<gst::Buffer, BevyError> {
        let mut buffer = gst::Buffer::new();
        let gst_format = dma_drm_fourcc_to_format(self.drm_format.code as u32)?;

        // let gst_format = fourcc_to_gst_format(self.drm_format.code)
        //     .ok_or_else(|| format!("Unable to convert drm format to gst format"))?;

        {
            let buffer_ref = buffer
                .get_mut()
                .ok_or_else(|| format!("Unable to get buffer ref"))?;

            let allocator = gst_allocators::DmaBufAllocator::new();
            let total_size = self
                .planes
                .iter()
                .map(|p| (p.offset + p.stride * self.height()) as usize)
                .max()
                .unwrap_or(0) as usize;

            let fd = self.open_fd()?;
            let mem = unsafe { allocator.alloc_dmabuf(fd, total_size)? };

            buffer_ref.append_memory(mem);
    ...
    pub fn build_gst_buffer_gemini(&self) -> Result<gst::Buffer, BevyError> {
        let gst_format = fourcc_to_gst_format(self.drm_format.code).ok_or_else(|| {
            format!(
                "DRM format {:?} not supported by GStreamer",
                self.drm_format.code
            )
        })?;
        ...
        let buffer_ref = buffer
            .get_mut()
            .ok_or_else(|| format!("Failed to get buffer ref"))?;

        // Import DMABuf
        let allocator = gst_allocators::DmaBufAllocator::new();

        // Compute total size (use offset of last plane + theoretical size)
        let total_size = self
            .planes
            .iter()
            .map(|p| (p.offset + p.stride * self.height()) as usize)
        ...
        let fd = self.open_fd()?; // Get a new handle
        let mem = unsafe { allocator.alloc_dmabuf(fd, total_size)? };
        buffer_ref.append_memory(mem);

        // Add video metadata
        let offsets: Vec<usize> = self.planes.iter().map(|p| p.offset as usize).collect();
        let strides: Vec<i32> = self.planes.iter().map(|p| p.stride as i32).collect();
...
unsafe fn create_image(
    ...
    info!("Available DRM format modifiers");
...
use gst_video::{VideoFormat, VideoInfoDmaDrm, dma_drm_fourcc_to_format};
fn fourcc_to_gst_format(code: DrmFourcc) -> Option<VideoFormat> {
    match code {
        // Formats RGB standards (8-bit)
        DrmFourcc::Argb8888 => Some(VideoFormat::Bgra),
        DrmFourcc::Xrgb8888 => Some(VideoFormat::Bgrx),
        DrmFourcc::Abgr8888 => Some(VideoFormat::Rgba),
        DrmFourcc::Xbgr8888 => Some(VideoFormat::Rgbx),
        DrmFourcc::Rgba8888 => Some(VideoFormat::Abgr),
        DrmFourcc::Rgbx8888 => Some(VideoFormat::Xbgr),
        DrmFourcc::Bgra8888 => Some(VideoFormat::Argb),
        DrmFourcc::Bgrx8888 => Some(VideoFormat::Xrgb),
        DrmFourcc::Rgb888 => Some(VideoFormat::Bgr),
        DrmFourcc::Bgr888 => Some(VideoFormat::Rgb),

        // Formats 16-bit (RGB565, etc.)
        DrmFourcc::Rgb565 => Some(VideoFormat::Rgb16),
        DrmFourcc::Bgr565 => Some(VideoFormat::Bgr16),

        // Formats YUV Planaires
        DrmFourcc::Yuv420 => Some(VideoFormat::I420),
        DrmFourcc::Yvu420 => Some(VideoFormat::Yv12),
        DrmFourcc::Yuv422 => Some(VideoFormat::Y42b),
        DrmFourcc::Yuv444 => Some(VideoFormat::Y444),

        // Formats YUV Semi-Planaires (Très utilisés en DMABuf/NVENC)
        DrmFourcc::Nv12 => Some(VideoFormat::Nv12),
        DrmFourcc::Nv21 => Some(VideoFormat::Nv21),
        DrmFourcc::Nv16 => Some(VideoFormat::Nv16),
        DrmFourcc::Nv61 => Some(VideoFormat::Nv61),
        DrmFourcc::Nv24 => Some(VideoFormat::Nv24),

        // Formats YUV Packets (Interleaved)
        DrmFourcc::Yuyv => Some(VideoFormat::Yuy2),
        DrmFourcc::Yvyu => Some(VideoFormat::Yvyu),
        DrmFourcc::Uyvy => Some(VideoFormat::Uyvy),
        DrmFourcc::Vyuy => Some(VideoFormat::Vyuy),
        DrmFourcc::Ayuv => Some(VideoFormat::Ayuv),
        DrmFourcc::Xyuv8888 => Some(VideoFormat::Ayuv),

        // Formats Haute Profondeur (10-bit)
        DrmFourcc::P010 => Some(VideoFormat::P01010le),
        DrmFourcc::P016 => Some(VideoFormat::P016Le),
        DrmFourcc::Abgr2101010 => Some(VideoFormat::Abgr64Le), // Approximation

        // Formats Mono / Greyscale
        DrmFourcc::R8 => Some(VideoFormat::Gray8),
        DrmFourcc::R16 => Some(VideoFormat::Gray16Le),

        // Formats spécifiques
        DrmFourcc::Vuy888 => Some(VideoFormat::V308),

        // Tous les autres formats (non supportés ou exotiques)
        _ => {
            bevy_log::warn!(
                "DRM Fourcc {:?} n'a pas d'équivalent direct dans GStreamer",
                code
            );
            None
        }
