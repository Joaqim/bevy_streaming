...
use bevy_render::render_graph::{RenderGraph, RenderGraphExt};
use gst::prelude::*;
...
use crate::pixelstreaming::controller::PSControllerState;
use crate::pixelstreaming::handler::PSMessageHandler;
use crate::surface::encoder::GstWebRtcDmabufEncoder;
use crate::surface::node::{SurfaceLabel, SurfaceNode, Viewports, viewport_extract};
use crate::{ControllerState, GstWebRtcSettings, StreamerCameraBuilder, StreamerHelper};
pub use dmabuf::*;

pub struct SurfaceInitPlugin;

impl Plugin for SurfaceInitPlugin {
    fn build(&self, app: &mut App) {
        init_plugin(app);
    }
}

pub(super) fn surface_plugin(app: &mut App) {
    ...
    render_app.init_resource::<Viewports>().add_systems(
        Render,
        (
    ...
    // graph.add_node(SurfaceLabel, SurfaceNode);
    // graph.add_node_edge(bevy_render::graph::CameraDriverLabel, SurfaceLabel);
}
...
pub struct GtkViewport {
    ...
    pub(crate) widget_scale_factor: Arc<AtomicF64>,
}
...
struct ViewportPrivate {
    ...
    encoder: Arc<GstWebRtcDmabufEncoder>,
}
...
struct RenderViewport {
    ...
    back_buffer: Vec<(Texture, TextureView)>,
    ...
    ///     `next_dmabuf`
    queued_dmabuf: Vec<DmabufTexture>,
...
impl GtkViewports<'_, '_> {
    ...
    pub fn create(&mut self, settings: GstWebRtcSettings) -> impl Bundle {
        let image_handle = self.images.reserve_handle();
        let next_dmabuf = Arc::new(AtomicOptionBox::none());
        let widget_size = Arc::new((
            AtomicU32::new(settings.width),
            AtomicU32::new(settings.height),
        ));
        let widget_scale_factor = Arc::new(AtomicF64::new(1.0));
        let widget_alive = Arc::new(());

        let encoder = GstWebRtcDmabufEncoder::with_settings(settings.clone())
...
impl ExtractComponent for RenderViewport {
    ...
    fn extract_component(viewport: QueryItem<Self::QueryData>) -> Option<Self::Out> {
        Some(Self {
            ...
            back_buffer: Vec::new(),
            old_widget_size: (u32::MAX, u32::MAX),
            queued_dmabuf: Vec::new(),
            current_buffer: 0,
            encoder: viewport.encoder.clone(),
            send_caps: true,
        })
    }
}
...
fn sync_viewport_and_camera(mut viewports: Query<(&GtkViewport, &mut RenderTarget)>) {
    for (viewport, mut render_target) in &mut viewports {
        *render_target = RenderTarget::Image(ImageRenderTarget {
            handle: viewport.image_handle.clone(),
            #[expect(clippy::cast_possible_truncation, reason = "we don't have a choice")]
            scale_factor: viewport.widget_scale_factor() as f32,
        });
    }
}
    ...
    for mut viewport in &mut viewports {
        ...
        if new_width != old_width || new_height != old_height {
            ...
            for _ in 0..3 {
                ...
                viewport.queued_dmabuf.push(dmabuf);
                viewport.back_buffer.push((texture, texture_view));
            }
            // let texture_view = texture.create_view(&TextureViewDescriptor::default());
        ...
        if let Some((texture, texture_view)) = &viewport.back_buffer.get(viewport.current_buffer) {
            let gpu_image = GpuImage {
                texture: texture.clone(),
                texture_view: texture_view.clone(),
                texture_format: texture.format(),
                // TODO is it correct to put None here ?
                texture_view_format: None,
                sampler: (**default_image_sampler).clone(),
                size: texture.size(),
                mip_level_count: 1,
                // TODO is it correct to put false here ?
                had_data: false,
            };
...
fn present_frames(mut viewports: Query<&mut RenderViewport>) {
    for mut viewport in &mut viewports {
        if let Some(dmabuf) = viewport.queued_dmabuf.get(viewport.current_buffer) {
            if viewport.send_caps {
                let caps = dmabuf.build_caps().expect("Unable to build dmabuf caps");
...
#[cfg(feature = "pixelstreaming")]
fn create_pixelstreaming_controller(encoder: &GstWebRtcDmabufEncoder) -> ControllerState {
    ...
    encoder
        ...
        .connect_closure("consumer-added", false, {
            ...
            glib::closure!(move |sink: &webrtcsink::BaseWebRTCSink,
                ...
                bevy_log::info!("New consumer: {}", peer_id);
    ...
    encoder
        ...
        .connect_closure("consumer-removed", false, {
            ...
            glib::closure!(move |_sink: &webrtcsink::BaseWebRTCSink,
                ...
                bevy_log::info!("Consumer removed: {}", peer_id);
